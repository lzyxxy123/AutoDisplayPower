...
- **界面风格**（参照 Microsoft Fluent Design + 火绒式"图标说话"）：
  - 文字统一近黑，**语义完全交给彩色矢量图标**（代码绘制，无需外部资源）：
    屏幕（仅外接=蓝显示器 / 仅笔记本=青笔记本 / 扩展=双显示器）、
    盖子（打开=绿笔记本 / 闭合=琥珀蚌壳 / 未知=灰）、
    策略（不操作=蓝暂停 / 睡眠=紫月亮 / 关机=琥珀电源 / 写入失败=红警告 / 未知=灰问号）
  - 菜单为浅色现代风格：淡蓝悬停、细边框、**当前模式整行浅色高亮**（不再用对钩）
  - **「开机自启动」为滑动开关**（轨道+圆钮），点击即切换
  - **电源策略变化时**：该行 3 秒内加粗 + 浅色高亮，并显示"（已更新）"
  - **托盘图标彩色并随屏幕状态变色**（仅外接=蓝 / 仅笔记本=青 / 扩展=紫 / 未知=灰）
  - **鼠标悬停托盘图标**显示单行摘要：`仅外接 · 盖子打开 · 合盖不操作`（系统提示仅支持单行）
  - 调试：`AutoDisplayPower.exe --icons` 可导出所有图标的 PNG 预览图
